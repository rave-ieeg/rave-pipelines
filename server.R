...
server <- function(input, output, session){
  ...
  is_single_session <- isTRUE(getOption(
    "ravedash.single.session",
    default = FALSE
  ))
  ...
  shiny::bindEvent(
    ravedash::safe_observe({
      ...
      resource <- shidashi::load_module(
        request = list(QUERY_STRING = query_string),
        env = parse_env
      )
          ...
          shiny::bindEvent(
            ravedash::safe_observe({
              ...
              ravedash::show_notification(
                ...
                type = "warning",
                message = sprintf(
                  "JupyterLab instance at port %s has been shut down",
                  conf$port
                ),
            ...
            ravedash::safe_observe(
              {
                ...
                if (length(conf$confpath) != 1 || is.na(conf$confpath)) {
                  stop(
                    "Unable to obtain the RAVE session path. Please contact the RAVE support team to report this bug."
                  )
                }
                ...
                ravedash::show_notification(
                  ...
                  message = sprintf(
                    "Starting JupyterLab at %s:%s ... (This might take a while if this is the first time that you configure the Python environment for RAVE)",
                    host,
                    jupyter_port
                  ),
                  close = FALSE,
              ...
              shiny::showModal(
                shiny::modalDialog(
                  ...
                  local({
                    if (jupyter_running) {
                      shiny::tagList(
                        "* If you shut down RAVE only, the active Jupyter server will still run in the background. Please manually stop the Jupyter server via ",
                        shiny::tags$span(
                          class = "font-weight-bold",
                          sprintf(
                            "rpymat::jupyter_server_stop(%s)",
                            jupyter_port
                          )
                        )
                      )
