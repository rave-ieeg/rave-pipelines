    ...
    input_subject_code = targets::tar_target_raw("subject_code",
        ...
        command = quote({
            .__target_expr__. <- quote({
                default_dcm2niix_path <- ravecore::cmd_dcm2niix(error_on_missing = FALSE)
                dcm2niix <- tryCatch({
                  dcm2niix <- ravecore::normalize_commandline_path(path = dcm2niix_path,
                ...
                default_fs_path <- ravecore:::cmd_freesurfer_home(error_on_missing = FALSE)
                freesurfer <- tryCatch({
                  freesurfer <- ravecore::normalize_commandline_path(path = freesurfer_path,
                ...
                default_fsl_path <- ravecore:::cmd_fsl_home(error_on_missing = FALSE)
                flirt <- tryCatch({
                  fsl <- ravecore::normalize_commandline_path(path = fsl_path,
                ...
                default_afni_path <- ravecore:::cmd_afni_home(error_on_missing = FALSE)
                afni <- tryCatch({
                  afni <- ravecore::normalize_commandline_path(path = afni_path,
        ...
        command = quote({
            .__target_expr__. <- quote({
                ...
                if (length(mri_path)) {
                  ...
                  ravepipeline::dir_create2(acpc_mri_dir)
        ...
        command = quote({
            .__target_expr__. <- quote({
                import_T1 <- tryCatch({
                  ravecore::cmd_run_dcm2niix(subject = subject,
                ...
                image_segmentation <- tryCatch({
                  ...
                  switch(paste(params$segmentation$program, collapse = ""),
                    `recon-all` = {
                      ...
                      ravecore::cmd_run_freesurfer_recon_all(subject = subject,
                      ...
                      ravecore::cmd_run_freesurfer_recon_all_clinical(subject = subject,
                      ...
                      ravecore::cmd_run_r(dry_run = TRUE, verbose = FALSE,
                        quoted = TRUE, expr = bquote({
                          subject <- ravecore::as_rave_subject(.(subject$subject_id))
        ...
        command = quote({
            .__target_expr__. <- quote({
                coreg_flirt <- tryCatch({
                  ...
                  ravecore::cmd_run_fsl_flirt(subject = subject,
                    mri_path = mri_path, ct_path = ct_path, dof = params$flirt$dof,
        ...
        command = quote({
            .__target_expr__. <- quote({
                coreg_nipy <- tryCatch({
                  ...
                  ravecore::cmd_run_nipy_coreg(subject = subject,
        ...
        command = quote({
            .__target_expr__. <- quote({
                coreg_niftyreg <- tryCatch({
                  ...
                  ravecore::cmd_run_niftyreg_coreg(subject = subject,
        ...
        command = quote({
            .__target_expr__. <- quote({
                coreg_ants <- tryCatch({
                  ...
                  ravecore::cmd_run_ants_coreg(subject = subject,
        ...
        command = quote({
            .__target_expr__. <- quote({
                coreg_3dallineate <- tryCatch({
                  ...
                  ravecore::cmd_run_3dAllineate(subject = subject,
        ...
        command = quote({
            ...
            "subject")), deps = c("params", "check_result", "subject"
        ), cue = targets::tar_cue("always"), pattern = NULL,
        iteration = "list"))
