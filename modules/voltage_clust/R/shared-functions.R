...
# static linking to the cache folder - when running this script, the working directory is the enclosing folder
cache_folder <- file.path(normalizePath("..", mustWork = TRUE, winslash = "/"), "data", "cache", fsep = "/")

ensure_cache_folder <- function() {
  return(ravepipeline::dir_create2(cache_folder))
}

voltage_baseline <- function(repository, baseline_window_cleaned) {

  baseline_window_cleaned <- ravecore::validate_time_window(as.double(unname(unlist(baseline_window_cleaned))))

  cache_path <- file.path(ensure_cache_folder(), "voltage_baseline")

  container <- repository$get_container()

  array_dim <- as.integer(unname(unlist(container$dim)))
  ...
  arr <- filearray::filearray_load_or_create(
    ...
    # additional headers
    baseline_window = baseline_window_cleaned,

    repository_signature = repository$signature,

    rave_data_type = "voltage-baselined",

    on_missing = function(arr) {

      # repository is LFP-only
      lapply(seq_along(container$data_list), function(ii) {
        # ii <- 1
        inst <- container$data_list[[ii]]
        channel_data <- inst[drop = FALSE, dimnames = FALSE]
        dim(channel_data) <- array_dim[c(1, 2)]
        channel_data <- t(channel_data) - colMeans(channel_data[baseline_time_selector, , drop = FALSE])
        arr[, , ii] <- t(channel_data)
        return()
      })

      dimnames(arr) <- container$dimnames
...
#' @param baseline_voltage baseline-corrected voltage
...
calc_similarity_matrix <- function(
  ...
  similarities <- lapply(seq_along(trial_numbers), function(trial_ii) {
    ...
    trial_voltage <- subset(baseline_voltage,
                          Trial ~ Trial == trial_number,
                          Time ~ Time >= start_time & Time <= end_time,
                          drop = FALSE)
    dimnames(trial_voltage) <- NULL
    trial_voltage[is.na(trial_voltage)] <- 0

    # -> time & trial X channel
    dm <- dim(trial_voltage)
    dim(trial_voltage) <- c(length(trial_voltage) / dm[[3]], dm[[3]])

    # Standardize
    norm <- sqrt(colSums(trial_voltage^2, na.rm = TRUE))
    norm[norm == 0] <- 1
    trial_voltage_standardized_t <- t(trial_voltage) / norm

    # Similarity (V^tilde %*% V)
    trial_voltage_standardized_t %*% trial_voltage
  })
...
calc_average_responses <- function(
  ...
  time_points <- dimnames(baseline_voltage)$Time
  ...
  lapply(seq_along(trial_numbers), function(trial_ii) {
    ...
    env$trial_voltage <- env$trial_voltage + re / length(trial_numbers)
    return()
  })

  # trial_voltage <- simplify2array(trial_voltage)
  #
  # ravetools::collapse(trial_voltage, keep = c(1, 2))
  env$trial_voltage
}

#' Prepare data for given conditions and epoch events or duration
#' @param repository,baseline_voltage pipeline targets
