diagnose_cluster <- function(cluster_result, k, combined_group_results, col_label = threeBrain:::DEFAULT_COLOR_DISCRETE) {
  ...
  gap_heights <- rev(seq_along(channel_order)) * zlim
  gapped_channel <- sweep(average_responses[, channel_order, drop = FALSE], 2L, gap_heights, "+")

  graphics::matplot(
    x = seq_len(nrow(average_responses)),
    y = gapped_channel,
    lty = 1, type = "l", ylim = c(0, length(channel_order) * zlim) + zlim / 2,
    xlab = "Time (s)", ylab = "Channel", main = sprintf("Cluster k=%d", result$k),
    axes = FALSE, xaxs = "i",
    col = channel_color_reordered
  )

  gapped_heights <- array(rep(gap_heights, each = nrow(gapped_channel)), dim = dim(gapped_channel))
  gapped_heights[gapped_heights < gapped_channel] <- NA_real_
  ...
  if(length(result$cluster) > 1) {
    separators <- cumsum(rev(table(result$cluster)))
    separators <- separators[-length(separators)]
    abline(h = (separators + 0.5) * zlim)
  }

  channel_y_at <- rev(seq_along(channels) * zlim)
...
diagnose_cluster <- function(cluster_result, k, combined_group_results, col_label = threeBrain:::DEFAULT_COLOR_DISCRETE) {
  # cluster_result <- clustering_tree
  # k <- 2
  # col_label = threeBrain:::DEFAULT_COLOR_DISCRETE
  average_responses <- combined_group_results$combined_average_responses
  result <- calc_final_cut_tree(cluster_result, k, cvi = FALSE)
