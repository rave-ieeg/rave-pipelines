...
```{r get_valid_project_name, engine = "rave", export = "loaded_brain_info", cue = "always", collapse=FALSE}
#| code-fold: true
#| code-summary: "Show a complicated implementation"

loaded_brain_info <- load_brain_from_subject_code(
...
initial_brain_widget <- loaded_brain_info$brain$plot(
  show_modal = FALSE,
...
path_datatable <- switch(
  ...
  "Saved pipelines/modules" = {
    ...
    pipepath <- get_subject_imaging_datapath(
      saved_pipeline, subject_code = loaded_brain_info$subject_code,
...
if (is.data.frame(loaded_datatable) && "Electrode" %in% nms) {
  ...
  if ("Subject" %in% nms) {
    ...
    if (!identical(loaded_brain_info$brain$subject_code, 
                  template_subject)) {
      loaded_datatable <- loaded_datatable[loaded_datatable$Subject %in% loaded_brain_info$brain$subject_code, ]
    }
  }
