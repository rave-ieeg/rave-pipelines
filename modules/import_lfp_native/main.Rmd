...
if(!is.numeric(format)) {
  format <- which(startsWith(names(IMPORT_FORMATS), trimws(format)))
}
...
if(isTRUE(skip_validation)) {
  ...
  validate_channels <- function(channels) {
    ...
    validation_result <- validate_raw_file(
    ...
    if(!validation_result) {
      ...
      stop('The following issues found when validating subject ',
           ...
           '.\n', msg, call. = bquote(validate_raw_file(
        subject_code = .(subject$subject_code),
        blocks = .(blocks), electrodes = .(channels),
        format = .(format)
      )))
...
subject$set_default(
  ...
  value = list(
    ...
    format = format,
    format_readable = names(IMPORT_FORMATS)[[format]],
    conversion = lfp_unit,
...
preprocess_info <- RAVEPreprocessSettings$new(subject$subject_id, read_only = TRUE)
