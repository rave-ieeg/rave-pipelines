    ...
    input_skip_validation = targets::tar_target_raw("skip_validation",
        ...
        command = quote({
            .__target_expr__. <- quote({
                subject <- RAVESubject$new(project_name = import_setup__project_name,
        ...
        command = quote({
            .__target_expr__. <- quote({
                ...
                if (!is.numeric(format)) {
                  format <- which(startsWith(names(IMPORT_FORMATS),
                    trimws(format)))
                }
                ...
                if (isTRUE(skip_validation)) {
                  ...
                  validate_channels <- function(channels) {
                    ...
                    validation_result <- validate_raw_file(subject_code = subject$subject_code,
                    ...
                    if (!validation_result) {
                      ...
                      stop("The following issues found when validating subject ",
                        ...
                        msg, call. = bquote(validate_raw_file(subject_code = .(subject$subject_code),
                          blocks = .(blocks), electrodes = .(channels),
                          format = .(format))))
                    }
                    validation_result
        ...
        command = quote({
            .__target_expr__. <- quote({
                ...
                if (length(lfp_channels)) {
                  rave_import(project_name = subject$project_name,
                ...
                if (length(compose_params)) {
                  lapply_async(compose_params, function(item) {
                ...
                subject$set_default(namespace = module_id, key = "import_parameters",
                  value = list(project_name = subject$project_name,
                    ...
                    format = format, format_readable = names(IMPORT_FORMATS)[[format]],
                    ...
                    data_type = data_types, add = FALSE, timestamp = strftime(Sys.time(),
                      usetz = TRUE)))
                preprocess_info <- RAVEPreprocessSettings$new(subject$subject_id,
                  read_only = TRUE)
