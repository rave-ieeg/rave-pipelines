...
validate_raw_file_lfp <- function(subject_code, blocks, electrodes, format, check_content = TRUE, ...){
  ...
  if(is.null(m)){
    ...
    validate_func <- sprintf("validate_raw_file_lfp.%s", m)
    do.call(validate_func, list(subject_code = subject_code, blocks = blocks, electrodes = electrodes,
...
validate_raw_file_lfp.native_matlab <- function(
  ...
  if( check_content && length(electrodes) ){
    progress <- ravepipeline::rave_progress('Check electrode files within block', shiny_auto_close = TRUE, max = length(blocks) + 1)
    for(b in blocks){
      progress$inc(b)
      info <- finfo[[b]]
      files <- info$files
