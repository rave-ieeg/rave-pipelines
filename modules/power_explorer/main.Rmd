...
# set 4 GiB export size? // set this based on RAM?
...
emmeans::emm_options(lmerTest.limit = 10290*10)
emmeans::emm_options(pbkrtest.limit = 10290*10)
...
# we don't need "Freq" because that is part of AnalysisLabel
...
condition_means <- data.table::as.data.table(dd)[ ,list('y'=mean(y), 'sd' = sd(y),
                                                        'se'=rutabaga:::se(y),'n'=.N), keyby=fe]

## single electrode in a single block
if(is.null(re_str)) {
  
  frm <- as.formula(
    sprintf('y ~ %s', paste0(fe, collapse='*'))
  )
  ...
  # here we have some kind of repeated measure
  f_string <- paste(paste(fe, collapse='*'), re_str, sep=' + ')
...
em <- emmeans::emmeans(
  mod, as.formula(sprintf(" ~ %s", paste(fe, collapse='*'))), infer=c(F,T)
)
