...
by_frequency_over_time_data <- do.call(
  ...
  sapply(analysis_settings_clean, function(asc) {
    ...
    collapse_e <- raveio::collapse2(baselined_power_data, keep = 1:3, method = "mean")
...
over_time_by_electrode_data <- do.call(
  ...
  sapply(analysis_settings_clean, function(asc) {
    ...
    collapse_f <- raveio::collapse2(baselined_power_data[fi,,,,drop=FALSE], keep = 2:4, method = "mean")
    # 
    # # add back in the 1st dimension (freq) b/c the shift function expects it
    dim(collapse_f) = c(1, dim(baselined_power_data)[2:4])
    dimnames(collapse_f) <- append(list('Frequency'='Avg'), dimnames(baselined_power_data)[2:4])
...
if(length(over_time_by_electrode_data) > 0) {
  curr_electrodes <- subset(repository$electrode_table, Electrode %in% over_time_by_electrode_data[[1]]$electrodes)
}
...
if(nrow(curr_electrodes) < 3) {
  ...
  by_electrode_similarity_data <- lapply(all_dist_mats, function(x) {
...
over_time_by_trial_data <- do.call(
  ...
  sapply(analysis_settings_clean, function(asc) {
    ...
    collapse_fe <- raveio::collapse2(baselined_power_data[fi,,,,drop=FALSE], keep = 2:3, method = "mean")
    # 
    # # add back in the 4th dimension (electrode) b/c the shift function expects it
...
if(length(all_data_clean) > 1) {
  ...
  if(mean(aggregate(Trial ~ Electrode, length, data=all_data_clean)$Trial) > 1.5) {
    
    data_by_el <- split(all_data_clean,all_data_clean$Electrode)
    stats <-  rutabaga::cbind_list(
      # raveio::lapply_async(data_by_el,FUN = run_stats)
