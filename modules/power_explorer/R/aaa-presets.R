  ...
  loader_subject_id = "loader_subject_code")
{
  ...
  comp$ui_func <- function(id, value, depends) {
    ...
    ravedash::flex_group_box(title = label, shidashi::flex_item(size = 3,
      shiny::p("Pre must be negative if you need a pre-stimulus baseline window"),
      ...
      shidashi::flex_item(shiny::numericInput(inputId = comp$get_sub_element_id("trial_starts",
        with_namespace = TRUE), label = "Pre", min = 0,
        value = pre)), shidashi::flex_item(shiny::numericInput(inputId = comp$get_sub_element_id("trial_ends",
          with_namespace = TRUE), label = "Post", min = 0,
          value = post)), shidashi::flex_break(), shidashi::flex_item(shinyWidgets::prettyCheckbox(inputId = comp$get_sub_element_id("default",
  ...
  comp$server_func <- function(input, output, session) {
    ...
    shiny::bindEvent(observe({
      ...
      watch_loader_opened(session = session), ignoreNULL = TRUE)
  }
  comp
}
