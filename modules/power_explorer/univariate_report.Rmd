...
ravepipeline::pipeline_set_preferences("power_explorer.graphics.reset_on_load" = FALSE)
...
re <- lapply(params$plots_to_include, call_plot_from_name, pipe=pipeline)
...
report_pipeline <- pipeline$fork(path=temp_root)
report_pipeline$clean('all')
...
pipe_res <- lapply(all_els, function(ee, settings=orig.settings) {
  ...
  report_pipeline$set_settings(.list = settings)
  
  utils::capture.output({
    res <- report_pipeline$run(names = data_needed, debug = FALSE, async =FALSE)
  })
...
for(ee in seq_along(all_els)) {
  ...
  lapply(params$plots_to_include, call_plot_from_name, pipe = pipe_res[[ee]])
