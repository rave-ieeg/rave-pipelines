...
if (!dir.exists(subject_root)) {
  ravepipeline::dir_create2(subject_root)
}

if (file.exists(migrate_result_path)) {
  migrate_result <- tryCatch({
    as.list(ravepipeline::load_yaml(migrate_result_path))
  }, error = function(e) {
    list()
  })
} else {
...
if (any(migrate_plan$Planned)) {
  ...
  res <- dipsaus::lapply_async2(
    ...
    FUN = function(ii) {
      ...
      if (file.exists(dst_path)) {
        if (backup) {
          ravecore::backup_file(dst_path, remove = TRUE)
        }
      }
      unlink(dst_path, recursive = TRUE)
      ravepipeline::dir_create2(dst_path)
  ...
  ravepipeline::save_yaml(migrate_result, file = migrate_result_path)
}
```
