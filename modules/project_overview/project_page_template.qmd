...
get_cache_file <- function(file_name) {
    ...
    list(
        ...
        link_tag = function(text) {
            glue::glue(
                "<a class=\"nowrap\" href=\"../cache/{project_name}/{file_name}\" target=\"viewer_iframe\" onclick=\"showIframeDrawer()\">{text}</a>"
            )
        }
    )
}
...
knitr::kable(dplyr::bind_rows(errors), format = "simple")
```
...
<script>
  function showIframeDrawer() {
    document.getElementById('iframe-drawer').classList.add('open');
  }
