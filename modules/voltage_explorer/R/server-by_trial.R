...
server_expr_by_trial <- function(input, output, session, ...) {
  ...
  shidashi::register_output(
    outputId = "figure_data_crp_param_by_trial_channel_heatmap",
    description = "CRP parameter (alpha prime) as a trial by channel heatmap, one panel per condition group.",
    download_type = "image",
    session = session,
