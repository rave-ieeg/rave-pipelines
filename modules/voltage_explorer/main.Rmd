...
```{r prepare_data_repository, engine='rave', language = "R", export = "repository", cue = "always"}
...
for (cfg in configs) {
  ...
  if (!is.null(f)) {
    ...
    xlim <- range(c(xlim, f$parameters$stopband * new_srate / 2), na.rm = TRUE)
  }
}
...
# lapply(analysis_electrodes_clean, function(ch) {
...
ravepipeline::with_rave_parallel({
  ravepipeline::lapply_jobs(analysis_electrodes_clean, function(ch) {
    sel <- repository$electrode_list %in% ch
    source_array <- repository$voltage$data_list[[which(sel)]]
    signals <- source_array[reshape = dim(source_array)[c(1, 2)]]
