...
has_surfaces <- vapply(from_subjects, function(subject) {
  subject <- ravecore::RAVESubject$new(project_name = project$name, subject_code = subject, strict = FALSE)
...
suppressWarnings({
  ravepipeline::with_rave_parallel({
    ...
    apply_mapping <- switch (
      mapping_method,
      'high-density' = ravecore::transform_thinfilm_to_mni152
    )
    
    mapped_results <- ravepipeline::lapply_jobs(
