...
source(testthat::test_path(
  "..", "..", "modules", "voltage_explorer", "R", "shared-selector_filter_electrodes.R"
), local = TRUE)
...
test_that("ALL requires every condition, ANY requires one", {
  ...
  expect_equal(
    selector_filter_electrodes(TBL, list(filter_row("all:al_p", "200"))),
    4
  )
  expect_equal(
    selector_filter_electrodes(TBL, list(filter_row("any:al_p", "200"))),
    c(2, 3, 4, 5)
  )
})
...
test_that("an aggregate matches the per-column chain it replaces", {
  ...
  expect_equal(
    selector_filter_electrodes(TBL, list(filter_row("any:al_p", "200"))),
    selector_filter_electrodes(TBL, list(
...
test_that("a missing value fails its own condition only", {
  ...
  expect_false(5 %in% selector_filter_electrodes(TBL, list(filter_row("all:al_p", "200"))))
  expect_true(5 %in% selector_filter_electrodes(TBL, list(filter_row("any:al_p", "200"))))
})

test_that("single-column rows are unchanged", {

  expect_equal(
    selector_filter_electrodes(TBL, list(filter_row("al_p (A)", "200"))),
    c(2, 4)
  )
...
test_that("unusable components are skipped, not errors", {
  ...
  expect_null(selector_filter_electrodes(TBL, list(filter_row("all:nope", "200"))))
  expect_null(selector_filter_electrodes(TBL, list(filter_row("", "200"))))
  expect_null(selector_filter_electrodes(TBL, list(filter_row("any:onset", "0.1"))))
  expect_null(selector_filter_electrodes(TBL, list(
    filter_row("all:al_p", "200", criteria = "in")
  )))
  expect_null(selector_filter_electrodes(TBL, list(filter_row("all:al_p", ""))))
...
test_that("selector_filter_choices puts the aggregates first", {

  choices <- selector_filter_choices(names(TBL), metrics = METRICS)
  ...
  expect_false(any(c("Electrode", "selector_filter") %in% choices))
})

test_that("selector_filter_choices omits aggregates for a single condition", {

  choices <- selector_filter_choices(
...
test_that("selector_filter_choices pushes onset to the end", {

  choices <- selector_filter_choices(c(
