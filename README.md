...
A `settings.yaml` is just a collection of key-value pairs. Module users are only allowed to set those inputs, hence consider carefully which inputs you want the users to alter. I recommend the RAVE function `ravepipeline::save_yaml` to programmically write to this file to avoid `yaml` syntax errors. 
...
repository <- ravecore::prepare_subject_power_with_epochs(subject_id)
```

```{rave calculate_baseline, language = "R", export = "baseline", cue = "always"}
ravecore::power_baseline(repository, c(0,0.5))
```
````
...
```{rave load_subject_power_data, language = "R", export = "repository", cue = "always"}
repository <- ravecore::prepare_subject_power_with_epochs(subject)

# or repository <- ravecore::prepare_subject_power_with_epochs(subject$subject_id)
```
